...
bitops_uses_same_shift_imm (ATTRIBUTE_UNUSED gimple *ior_stmt, ATTRIBUTE_UNUSED gimple *and_stmt)
{
  ...
  if (TREE_CODE (rhs1) == SSA_NAME
      ...
      && TYPE_UNSIGNED (TREE_TYPE (gimple_assign_lhs (ior_stmt))))
    {
      ssa_name = rhs1;
      ior_imm = rhs2;
    }
  ...
  if (TREE_CODE (rhs1) == SSA_NAME
      ...
      && TYPE_UNSIGNED (TREE_TYPE (rhs1))
      && TYPE_UNSIGNED (TREE_TYPE (rhs2))
      && TYPE_UNSIGNED (TREE_TYPE (gimple_assign_lhs (and_stmt))))
    {
      if (rhs1 != ssa_name)
	return 0;
		      ...
		      unsigned HOST_WIDE_INT result_imm)
{
  ...
  gimple *one_stmt = gimple_build_assign (one_set,
	wide_int_to_tree (elems_type, GET_MODE_MASK (TYPE_MODE (elems_type))));
  ...
  if (op2_stmt)
  ...
  else
    {
      ...
      if (e1->dest == phi->bb)
	SET_PHI_ARG_DEF (phi, e1->dest_idx,
			 wide_int_to_tree (elems_type, 0));
      else
      	SET_PHI_ARG_DEF (phi, e2->dest_idx,
			 wide_int_to_tree (elems_type, 0));
    }
  ...
  gimple *result_stmt = gimple_build_assign (lshift, BIT_AND_EXPR, gphi_res,
					     result_imm_tree);
