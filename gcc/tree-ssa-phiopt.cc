...
{
  gimple *stmt = last_and_only_stmt (bb);
...
/* Helper function of canonicalize_conditional_ops to check if
   a given conditional stmt can be canonicalized, i.e. moved to
   the merge block.  */
...
/* Helper function of canonicalize_conditional_ops that moves
   a conditional statement 'op1_stmt' from middle_bb to the phi
   block.

   The steps are:
   - create a new PHI.  The edge from middle_bb to merge_bb have a
   '1' value, the edge from cond_bb to merge_bb have a 0 value;
   - add a new statement that recreates the immediate used in
   the operation from op1_stmt using the PHI result (immediate_stmt);
   - move 'op1_stmt' from middle_bb to merge_bb.  op1_stmt will now
   use LHS (immediate_stmt) as its RHS2;
   - finally, replace the original PHI result with LHS (op1_stmt).  */
		      ...
		      unsigned HOST_WIDE_INT imm)
{
  ...
  tree mult = make_ssa_name (immediate_type);
  tree imm_tree = wide_int_to_tree (immediate_type, imm);
  immediate_stmt = gimple_build_assign (mult, MULT_EXPR,
...
/* The transformation that is attempted here is to turn a
   conditional operation in the format

   ssa2 = ssa1 OP immediate

   to be unconditional.  This is done by moving the operation
   from middle_bb to merge_bb and reconstructing the
   immediate by using the PHI result that is canonicalized
   to 0/1.  For example this pattern:

   cond_bb:
   if (cond) goto middle_bb else goto merge_bb
   ...
   A common result of this pattern is the gcond disappearing,
   together with the PHI, and the immediate is re-created
   using 'cond' directly and everything being done in a single
   block:

   cond_bb:
   bool_val = cond
   immediate = bool_val << 3;
   ssa2 = ssa1 | immediate;
   (...)

   Note that this function is just a precondition check.  The
   main work is done in 'move_conditional_ops'.  */
